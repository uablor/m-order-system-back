...
export class RolePermissionRepository {
  ...
  async add(roleId: number, permissionId: number, manager?: EntityManager): Promise<RolePermissionOrmEntity> {
  ...
  async remove(roleId: number, permissionId: number, manager?: EntityManager): Promise<boolean> {
    const repo = manager ? manager.getRepository(RolePermissionOrmEntity) : this.repository;
    const result = await repo.delete({ roleId, permissionId });
    return (result.affected ?? 0) > 0;
  }

  async exists(roleId: number, permissionId: number, manager?: EntityManager): Promise<boolean> {
    const repo = manager ? manager.getRepository(RolePermissionOrmEntity) : this.repository;
